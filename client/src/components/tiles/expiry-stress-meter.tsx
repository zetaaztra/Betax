...
export function ExpiryStressMeter({ data, testId }: ExpiryStressMeterProps) {
  const { score, level } = data.expiry_stress;
  
  const getColor = () => {
    if (level === "LOW") return { text: "text-bullish", bg: "bg-bullish/20", stroke: "stroke-bullish" };
    if (level === "MEDIUM") return { text: "text-neutral", bg: "bg-neutral/20", stroke: "stroke-neutral" };
