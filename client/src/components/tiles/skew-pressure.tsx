...
export function SkewPressure({ data, testId }: SkewPressureProps) {
  ...
  const maxSkew = Math.max(put_skew, call_skew);
  const putHeight = (put_skew / maxSkew) * 100;
  const callHeight = (call_skew / maxSkew) * 100;
      ...
      <div className="space-y-4">
        ...
        <div className="text-center p-2 bg-muted rounded">
          ...
          <div className="text-sm font-semibold font-mono">
            {Math.abs(put_skew - call_skew).toFixed(1)} 
